...
class InviteUsersDatabase:
    ...
    async def batch_upsert_users(self, users: list):
        """Process users using separate update_one and save operations without upsert"""
        try:
            if not users:
            ...
            logger.info(
                f"💾 Processing {len(users)} users to invite_users database...")
            ...
            for user in users:
                ...
                user_data = {**user, "updated_at": datetime.now()}

                # Check if user exists
                existing_user = await self.invite_users.find_one({"uid": user_id})

                if existing_user:
                    # Update existing user
                    result = await self.invite_users.update_one(
                        {"uid": user_id},
                        {"$set": user_data}
                    )
                    ...
                    # Insert new user using save
                    await self.invite_users.insert_one(user_data)
                    inserted_count += 1

            logger.info(
                f"✅ Database processing completed: {inserted_count} inserted, {updated_count} updated")

        except Exception as e:
            logger.error(
                f"❌ Failed to process users in invite_users: {e}")
            raise
